---
title: Lab 2 Assignment
publish: true
---

>**Students:** Maxence DELEHELLE, Amine SAAD-EDDINE   
>**Teacher:** Badr TAJINI     
>**Academic year:** 2025–2026  
>**Program:** Data & Applications - Engineering - (FD)   
>**Course:** Data Engineering II 
---

## Dataset

- [Dataset_Track_D_csv](https://drive.google.com/file/d/18DUKA_9A6jSs6UzWWTI9VteLPBK-_y-W/view?usp=sharing)

## Notebook

- [Voir le notebook dédié](/lab_2_assignement/notebook)

## Metrics log

- [lab2_metrics_log.csv](lab2_metrics_log.csv)

## Outputs

- [inverted_index.csv](inverted_index.csv)

## Proof

- [metrics_traitement_1.png](metrics_traitement_1.png)
- [preuve_1.png](preuve1.png)

- [plan_index_build.txt](plan_index_build.txt)
- [plan_query.txt](plan_query.txt)